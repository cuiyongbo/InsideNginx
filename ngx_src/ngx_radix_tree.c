...
{
    ngx_radix_tree_t* tree = ngx_palloc(pool, sizeof(ngx_radix_tree_t));
    if (tree == NULL)
        return NULL;
...
static ngx_radix_node_t* ngx_radix_alloc(ngx_radix_tree_t* tree)
{
	if (tree->free_list)
	{
		ngx_radix_node_t* p = tree->free_list;
        tree->free_list = tree->free_list->right;
        return p;
    }
	...
	ngx_radix_node_t* p = (ngx_radix_node_t*)tree->start;
